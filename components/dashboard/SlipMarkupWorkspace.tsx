...
/**
 * Live slip-markup workspace. Renders a slip document on the left and an
 * accordion clause list on the right. Ticking a clause updates the slip in
 ...
 * The selection is persisted via save_submission_clauses and feeds the future
...
function SlipPreview({ clauses, checked, flash, row, locale }: { clauses: Clause[]; checked: Set<string>; flash: string | null; row: CaseLite; locale: string }) {
  const byCat = useMemo(() => {
...
function ClauseAccordion({ clauses, checked, expanded, onToggle, onExpand, locale }: {
